...
class Factor(object):
  ...
  def normalizeIP(self):
    np.divide(self.t, np.sum(self.t), out=self.t)
    return self

  def normalizeLogIP(self):
    np.subtract(self.t, sp.special.logsumexp(self.t), out=self.t)
    return self

  def normalizeLog(self):
    new_fact=self.copy()
    new_fact.normalizeLogIP()
    return new_fact
  ...
  def partial_sum(self, elim=None, out=None):
  ...
  def sumPower(self, elim=None, power=1.0, out=None):
    ...
    if (elim is None): elim = self.v
    tmp = (self ** power).partial_sum(elim)
    tmp **= (1.0/power)
    return tmp

  def lsemarginal(self, target, out=None):
    tmp = self.copy()
    maxtmp = np.max(tmp.table)
    tmp -= maxtmp
    tmp.expIP()
    return tmp.marginal(target).logIP() + maxtmp
  ...
  def distance(self, that, distance):
    ...
    elif distance == 'r2':   tmp -= that; tmp *= tmp; tmp /= that; return tmp.sum(); #tmp *= tmp; tmp /= that; return tmp.sum();
    else: raise ValueError("Unrecognized norm type {}; 'L1','L2','LInf','KL','HPM'".format(distance));
